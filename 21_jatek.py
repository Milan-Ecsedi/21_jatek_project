...
class Kartya:
    ...
    def ertekeles(self):
        if self.ertek == 14:  # Ász
            return 11
        elif self.ertek >= 11:  # Alsó, Felső, Király
            return 10
        return self.ertek
...
class Jatekos:
    ...
    def kez_ertek(self):
        ...
        while osszeg > 21 and aszok_szama:
            osszeg -= 10  # Ász értéke 1-re csökkentve
            aszok_szama -= 1
        return osszeg
...
class FeketeJatek:
    def __init__(self, root):
        ...
        self.current_tet = 0
        self.canvas = tk.Canvas(root, width=700, height=400, bg="#1F1F1F")
    ...
    def frissit_ablak(self):
        ...
        self.canvas.create_text(450, 50, text="Osztó", fill="white", font=("Arial", 16, 'bold'), anchor=tk.S)
        self.canvas.create_text(450, 80, text=f"Érték: {self.oszto.kez_ertek()}", fill="white", font=("Arial", 14), anchor=tk.S)

        ## Játékos értéke és neve (felül, bal oldalon)
        self.canvas.create_text(200, 50, text="Játékos", fill="white", font=("Arial", 16, 'bold'), anchor=tk.S)
        self.canvas.create_text(200, 80, text=f"Érték: {self.jatekos.kez_ertek()}", fill="white", font=("Arial", 14), anchor=tk.S)
        ...
        for kartya in self.jatekos.kartyak:
            ...
            self.canvas.create_image(150, y_offset, image=kartya_kep, anchor=tk.W)
            self.canvas.create_text(220, y_offset, text=str(kartya), fill="white", font=("Arial", 14), anchor=tk.W)
        ...
        # Kép és szöveg elhelyezése egymás mellett
            self.canvas.create_image(450, y_offset, image=kartya_kep, anchor=tk.W)
            self.canvas.create_text(520, y_offset, text=str(kartya), fill="white", font=("Arial", 14), anchor=tk.W)
            y_offset += 80  # Sor eltolása

        self.canvas.create_text(350, 300, text=f"Egyenleg: {self.jatekos.egyenleg} Ft", fill="white", font=("Arial", 18, 'bold'))
